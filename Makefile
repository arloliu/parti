...
TEST_DIRS       := $(sort $(dir $(shell find . -name "*_test.go" -not -path "./vendor/*" -not -path "./.claude/*" -not -path "./test/integration/*" -not -path "./test/stress/*" -not -path "./test/perf-measurement/*" -not -path "./docs/plans/partition-scaling/poc/*" -not -path "./k8s/*" -not -path "./tmp/*")))
...
test-kpi-long: clean-test-results
	@echo "Running SC-P0-1 KPI test with trials=$(TRIALS)..."
	@mkdir -p docs/design/06-implementation
	@/usr/bin/env bash -c 'set -o pipefail; TRIALS=$(TRIALS) go test -v -run ^TestWorkerConsumer_ExternalDeletionRecoveryKPI$$ ./test/integration -timeout=$(STRESS_TIMEOUT) | tee docs/design/06-implementation/kpi_sc_p0_1_latest.txt'
	@echo "KPI output snapshot written to docs/design/06-implementation/kpi_sc_p0_1_latest.txt"
...
## inspect-consumers: Inspect JetStream pull consumer configurations for a stream (use STREAM env/var)
inspect-consumers:
	@if [ -z "$(STREAM)" ]; then echo "STREAM required: make inspect-consumers STREAM=<stream> [JSON=1 SUBJECTS=1]"; exit 2; fi
	@echo "Inspecting consumers for stream $(STREAM)..."
	@JSON_FLAG=""; [ "$(JSON)" = "1" ] && JSON_FLAG="-json"; SUBJECTS_FLAG=""; [ "$(SUBJECTS)" = "1" ] && SUBJECTS_FLAG="-subjects"; \
	go run ./scripts/inspect_consumers -stream $(STREAM) $$JSON_FLAG $$SUBJECTS_FLAG

## inspect-consumers-json: Convenience target for JSON output (STREAM required)
inspect-consumers-json:
...
gap-timeline:
	@if [ -z "$(DIR)" ]; then echo "DIR required: make gap-timeline DIR=/tmp/parti-sim-stress/latest [JSON=1 WINDOW=10]"; exit 2; fi
	@echo "Parsing gap timeline in $(DIR)..."
	@JSON_FLAG=""; [ "$(JSON)" = "1" ] && JSON_FLAG="-json"; WINDOW_FLAG=""; [ -n "$(WINDOW)" ] && WINDOW_FLAG="-window $(WINDOW)"; \
	go run ./scripts/gap_timeline -dir $(DIR) $$JSON_FLAG $$WINDOW_FLAG
