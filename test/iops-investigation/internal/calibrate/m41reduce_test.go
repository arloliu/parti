...
func TestReduceM41_LeaderFollowerSplit(t *testing.T) {
	// Leader = nats-1 → container iops-nats-1, deltas (200r, 100w)/s.
	// Followers = nats-2, nats-3 with (50r, 25w)/s and (30r, 15w)/s.
	cg := buildCgroup(map[string]struct{ R, W uint64 }{
		"iops-nats-1": {R: 200, W: 100},
		"iops-nats-2": {R: 50, W: 25},
		"iops-nats-3": {R: 30, W: 15},
	})
	driver := DriverRow{
		CStream:       500,
		FetchTimeoutS: 5,
...
func TestContainerToNode(t *testing.T) {
	require.Equal(t, "nats-1", ContainerToNode("iops-nats-1"))
	require.Equal(t, "nats-5", ContainerToNode("iops-nats-5"))
}
