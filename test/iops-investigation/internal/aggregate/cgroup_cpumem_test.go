...
func TestParseCgroupCPUMem(t *testing.T) {
	const raw = "# t_unix_ns container usage_usec memory_current_bytes\n" +
		"1000000000 iops-nats-1 500000 104857600\n" +
		"2000000000 iops-nats-1 1500000 110000000\n"
	...
	if got[0].Container != "iops-nats-1" || got[0].UsageUsec != 500000 || got[0].MemoryBytes != 104857600 {
...
func TestCPUMemDeltas_KnownFraction(t *testing.T) {
	...
	samples := []CPUMemSample{
		// tick 1: baseline.
		{TUnixNs: 10 * sec, Container: "iops-nats-1", UsageUsec: 1_000_000, MemoryBytes: 100},
		// tick 2: +500_000 usage_usec over a 1s wall gap ⇒ 0.5 core.
		{TUnixNs: 11 * sec, Container: "iops-nats-1", UsageUsec: 1_500_000, MemoryBytes: 200},
	}
...
func TestCPUMemDeltas_FullCoreAndMultiContainer(t *testing.T) {
	...
	samples := []CPUMemSample{
		...
		{TUnixNs: 10 * sec, Container: "iops-nats-1", UsageUsec: 0, MemoryBytes: 10},
		{TUnixNs: 11 * sec, Container: "iops-nats-1", UsageUsec: 1_000_000, MemoryBytes: 11},
		// container 2: +2_500_000µs over 1s ⇒ 2.5 cores.
		{TUnixNs: 10 * sec, Container: "iops-nats-2", UsageUsec: 0, MemoryBytes: 20},
		{TUnixNs: 11 * sec, Container: "iops-nats-2", UsageUsec: 2_500_000, MemoryBytes: 22},
	}
	deltas := CPUMemDeltas(samples)
	if len(deltas) != 2 {
	...
	if math.Abs(byC["iops-nats-1"].CPUCores-1.0) > 1e-9 {
		t.Errorf("nats-1 CPUCores = %v, want 1.0", byC["iops-nats-1"].CPUCores)
	}
	if math.Abs(byC["iops-nats-2"].CPUCores-2.5) > 1e-9 {
		t.Errorf("nats-2 CPUCores = %v, want 2.5", byC["iops-nats-2"].CPUCores)
	}
}
