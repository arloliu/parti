...
func TestIntegration_K8sRollingUpdate_PreservesAssignments(t *testing.T) {
	...
	initialUnique := make(map[string]struct{})
	...
	require.Equal(t, 100, len(initialUnique), "expected all 100 partitions assigned initially")
	...
	for i := 0; i < 10; i++ {
		...
		for time.Since(startWait) < maxWaitTime {
			...
			for j := 0; j < originalWorkerCount; j++ {
				...
				for _, p := range assignment.Partitions {
					for _, key := range p.Keys {
						uniqDuring[key] = struct{}{}
					}
				}
			}
			t.Logf("Partitions during replacement (after %v): %d", time.Since(startWait).Round(time.Second), len(uniqDuring))
			if len(uniqDuring) == 100 {
				stabilized = true
				break
			}
		}
	...
	require.Equal(t, 100, len(finalUnique), "expected all 100 partitions assigned after rolling update")
	...
	affinityPercent := float64(matchedPartitions) / float64(len(initialUnique)) * 100
	t.Logf("Cache affinity: %.1f%% (%d/%d partitions stayed with same worker)",
		affinityPercent, matchedPartitions, len(initialUnique))
...
func TestIntegration_HighChurn_SystemStable(t *testing.T) { //nolint:cyclop
	if testing.Short() {
		t.Skip("skipping integration test in short mode")
	}
	t.Parallel()
	...
	go func() {
		...
		for {
			...
			case <-monitorTicker.C:
				...
				t.Logf("Monitor: %d workers, %d unique partitions assigned", len(workers), len(uniq))

				// Prefer robustness under churn: log deficits but don't fail mid-churn
				// Final stabilization check below asserts complete coverage.
				if len(uniq) != 50 {
					t.Logf("CRITICAL: Only %d/50 unique partitions assigned during churn", len(uniq))
				}
			}
		}
	}()
	...
	for _, mgr := range cluster.Workers {
		assignment := mgr.CurrentAssignment()
		afterAssignments[mgr.WorkerID()] = assignment.Partitions
		t.Logf("Worker %s: %d partitions after churn", mgr.WorkerID(), len(assignment.Partitions))
	}
	afterUnique := make(map[string]struct{})
	for _, parts := range afterAssignments {
		for _, p := range parts {
			id := p.ID()
			if id == "" {
				continue
			}
			afterUnique[id] = struct{}{}
		}
	}
	require.Equal(t, 50, len(afterUnique), "expected all 50 partitions assigned after churn")
...
func TestIntegration_MassWorkerFailure_Recovers(t *testing.T) {
	...
	// Verify initial state (unique partition coverage across workers)
	initUniq := make(map[string]struct{})
	for _, mgr := range cluster.Workers {
		assignment := mgr.CurrentAssignment()
		for _, p := range assignment.Partitions {
			if id := p.ID(); id != "" {
	...
	require.Equal(t, 50, len(initUniq), "expected all 50 partitions assigned initially")
	...
	require.Eventually(t, func() bool {
		...
		if totalAfterFailure != 50 {
			t.Logf("Waiting for rebalance... current total partitions: %d", totalAfterFailure)
			return false
		}

		return true
	}, 30*time.Second, 1*time.Second, "expected survivors to handle all 50 partitions after mass failure")

	// Log final distribution for debugging
	for i := 8; i < 10; i++ {
		assignment := cluster.Workers[i].CurrentAssignment()
		t.Logf("Survivor worker %s: %d partitions", cluster.Workers[i].WorkerID(), len(assignment.Partitions))
	}
	...
	// Verify all partitions are assigned (unique across active workers)
	// Use GetActiveWorkers to exclude stopped workers
	activeWorkers := cluster.GetActiveWorkers()
	uniq := make(map[string]struct{})
	for _, mgr := range activeWorkers {
	...
	require.Equal(t, 50, len(uniq), "expected all 50 partitions assigned after recovery")
