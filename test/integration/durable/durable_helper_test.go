...
func TestSubscriptionHelper_ErrorHandling(t *testing.T) {
	...
	// Verify no new subjects were added (allow 0 if rollback removed local mapping)
	subjects = helper.WorkerSubjects()
	require.LessOrEqual(t, len(subjects), 1, "Should not add new subject on cancelled update")
	...
	// Should not have increased subjects due to invalid update
	subjects = helper.WorkerSubjects()
	require.LessOrEqual(t, len(subjects), 1, "Invalid update must not add subjects")
