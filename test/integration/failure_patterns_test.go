...
// uniquePartitionCount returns the number of unique partitions across workers,
// identified by the partition ID (dash-joined keys).
func uniquePartitionCount(assignments map[string][]types.Partition) int {
...
func TestPattern_ThunderingHerd_AllWorkersRestartSimultaneously(t *testing.T) {
	...
	initialUnique := uniquePartitionCount(initialAssignments)
	require.Equal(t, 50, initialUnique, "expected all 50 partitions assigned initially")
	...
	finalUnique := uniquePartitionCount(finalAssignments)
	require.Equal(t, 50, finalUnique, "expected all 50 partitions assigned after recovery")
	...
	affinityPercent := float64(matchedPartitions) / float64(initialUnique) * 100
	t.Logf("Cache affinity: %.1f%% (%d/%d partitions stayed with same worker)",
		affinityPercent, matchedPartitions, initialUnique)
...
func TestPattern_SplitBrain_DualLeaders(t *testing.T) {
	...
	afterUnique := uniquePartitionCount(afterAssignments)
	require.Equal(t, 50, afterUnique, "expected all 50 partitions assigned after checks")
	...
	finalAfterFail := make(map[string][]types.Partition)
	for _, mgr := range cluster.Workers {
		assignment := mgr.CurrentAssignment()
		finalAfterFail[mgr.WorkerID()] = assignment.Partitions
		t.Logf("Worker %s: %d partitions after failover", mgr.WorkerID(), len(assignment.Partitions))
	}
	finalAfterFailUnique := uniquePartitionCount(finalAfterFail)
	require.Equal(t, 50, finalAfterFailUnique, "expected all 50 partitions assigned after failover")
...
func TestPattern_GrayFailure_SlowButNotDead(t *testing.T) {
	...
	for time.Since(monitorStart) < 30*time.Second {
		...
		for _, mgr := range cluster.Workers {
			...
			// Allow small variations (±2) due to edge cases
			require.InDelta(t, initialCount, currentCount, 2.0,
				"Worker %s assignment changed unexpectedly: %d → %d",
				mgr.WorkerID(), initialCount, currentCount)
		}

		require.Equal(t, 50, len(uniq), "total partition coverage changed")
	}
	...
	finalUniq := make(map[string]struct{})
	for _, mgr := range cluster.Workers {
		assignment := mgr.CurrentAssignment()
		count := len(assignment.Partitions)
	...
	require.Equal(t, 50, len(finalUniq), "expected all 50 partitions assigned finally")
	...
	for workerID, initialCount := range initialAssignments {
		...
		require.InDelta(t, initialCount, finalCount, 2.0,
