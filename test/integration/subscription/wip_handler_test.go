...
// TestWIPHandler_WithoutHeartbeat_Redelivers verifies that without WIPHandler,
// a slow handler causes message redelivery when processing exceeds AckWait.
//
// This is the "control" test to confirm the WIP integration test is meaningful.
func TestWIPHandler_WithoutHeartbeat_Redelivers(t *testing.T) {
	if testing.Short() {
		t.Skip("integration: skipping in short mode")
	}
	...
	// Publish a test message
	_, err = js.Publish(ctx, "control.test", []byte("will-redeliver"))
	require.NoError(t, err)

	var processCount atomic.Int32

	// Slow handler WITHOUT WIP - will cause redelivery
	slowHandler := subscription.MessageHandlerFunc(func(c context.Context, msg jetstream.Msg) error {
		processCount.Add(1)
		// Sleep longer than AckWait - this will trigger redelivery
		time.Sleep(1500 * time.Millisecond)
		return nil
	})

	// Create worker consumer with very short AckWait (no WIP wrapper)
	helper, err := subscription.NewWorkerConsumer(js, subscription.WorkerConsumerConfig{
		StreamName:      streamName,
		ConsumerPrefix:  "control-worker",
		SubjectTemplate: "control.{{.PartitionID}}",
		AckWait:         1 * time.Second, // Very short - will redeliver
		BatchSize:       1,
		MaxDeliver:      3, // Allow up to 3 deliveries
	}, slowHandler)
	require.NoError(t, err)
	defer helper.Close(context.Background())

	// Start consumption
	require.NoError(t, helper.UpdateWorkerConsumer(ctx, "worker-ctrl", []parti.Partition{{Keys: []string{"test"}}}))

	// Wait for potential redeliveries
	time.Sleep(4 * time.Second)

	// Without WIP, the message should be redelivered multiple times
	count := processCount.Load()
	require.Greater(t, count, int32(1), "without WIP, message should be redelivered (got %d deliveries)", count)
}
