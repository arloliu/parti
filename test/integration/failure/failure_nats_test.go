...
func TestNATSFailure_AssignmentPublishRetry(t *testing.T) {
	...
	// Record initial assignments (unique coverage)
	initialAssignments := make(map[string]int)
	uniqInitial := make(map[string]struct{})
	for _, mgr := range cluster.Workers {
		assignment := mgr.CurrentAssignment()
		initialAssignments[mgr.WorkerID()] = len(assignment.Partitions)
		for _, p := range assignment.Partitions {
			if id := p.ID(); id != "" {
				uniqInitial[id] = struct{}{}
			}
		}
		t.Logf("Worker %s: %d partitions", mgr.WorkerID(), len(assignment.Partitions))
	}

	t.Logf("Total unique partitions assigned: %d", len(uniqInitial))
	require.Equal(t, 40, len(uniqInitial), "expected all 40 partitions assigned")
	...
	// Verify all workers have assignments (including the new one), unique coverage
	finalAssignments := make(map[string]int)
	uniqFinal := make(map[string]struct{})
	for _, mgr := range cluster.Workers {
		assignment := mgr.CurrentAssignment()
	...
	t.Logf("Total unique partitions assigned after rebalance: %d", len(uniqFinal))

	// Verify all partitions still assigned (no orphans)
	require.Equal(t, 40, len(uniqFinal), "expected all 40 partitions still assigned")
