...
func ContainerToNode(container string) string {
	return strings.TrimPrefix(container, "perf-")
}
...
// `perf-` prefix convention. If the input is the sentinel "single"
...
func nodeToContainer(node string) string {
	...
	if strings.HasPrefix(node, "perf-") {
