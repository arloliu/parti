...
#       Per-cell server.conf swap via PERF_RIG_NATS_CONFIG (7th _def_cell arg).
...
Environment:
  PERF_RIG_NATS_IMAGE         Docker image for the NATS cluster (default: nats:2.12.6).
  PERF_RIG_NATS_IMAGE_DIGEST  Pre-resolved image digest for manifest (optional).
...
write_campaign_manifest() {
...

    {
        ...
        echo "nats_image: ${PERF_RIG_NATS_IMAGE:-nats:2.12.6}"
    } > "$CAMPAIGN_MANIFEST"
}
...
run_one() {
    ...
    # Pass PERF_RIG_NATS_REPLICAS so the Makefile picks up the right profile.
    # Pass PERF_RIG_NATS_CONFIG when the cell defines a per-cell server.conf
    ...
    if ! PERF_RIG_NATS_REPLICAS="${replicas}" \
         PERF_RIG_NATS_CONFIG="${nats_config}" \
