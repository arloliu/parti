...
#     bring up with PERF_RIG_NATS_REPLICAS=3 once at start.
#   Cycle 2 (R=5): M4.1 R=5 chunk (24 grid points).
#     teardown+up with PERF_RIG_NATS_REPLICAS=5 before the R=5 chunk.
...
NATS_IMAGE="${PERF_RIG_NATS_IMAGE:-nats:2.12.6}"
NATS_IMAGE_DIGEST=""
...
if [[ "$RUN_M41" == "true" ]]; then
    ...
    PERF_RIG_NATS_REPLICAS=5 make -C "$RIG_DIR" reset
