...
#
# Method:
#   1. Bring up an perf-calibrator sidecar (busybox) on a fresh docker
...
CALIBRATOR_NAME="perf-calibrator"
...
CALIBRATOR_VOL="perf-calibrator-data"
...
(
    ...
    if ! docker compose -f docker/docker-compose.yaml --profile r3 ps --status running 2>/dev/null \
        | awk 'NR>1 {print}' | grep -q perf-nats-1; then
