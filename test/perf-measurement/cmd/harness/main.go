...
func parseFlags(args []string) (Options, error) {
	...
	var (
		...
		dataStream      = fs.String("data-stream-name", "perf-rig-data", "data stream name")
...
captureLoop:
	...
	return WriteManifest(o.OutputDir, Manifest{
		...
		NATSImage:                    os.Getenv("PERF_RIG_NATS_IMAGE"),
		NATSImageDigest:              resolveImageDigest(),
		RunIndex:                     os.Getenv("PERF_RIG_RUN_INDEX"),
...
// resolveImageDigest reads PERF_RIG_NATS_IMAGE_DIGEST if the operator
...
func resolveImageDigest() string {
	return os.Getenv("PERF_RIG_NATS_IMAGE_DIGEST")
}
